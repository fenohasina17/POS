...
// Map pour associer session_id -> cash_register_id
const sessionToCashRegisterMap = ref(new Map())
...
// Helper pour extraire un tableau depuis la réponse API (gère plusieurs formats)
...
// Récupération des ID normalisés (retourne une chaîne ou null)
const getRegisterId = (register) => {
  const id = register?.id ?? register?.cash_register_id ?? register?.cashRegisterId
  return id != null ? String(id) : null
}

const getPointOfSaleId = (pos) => {
  const id = pos?.id ?? pos?.point_of_sale_id ?? pos?.pointOfSaleId
  return id != null ? String(id) : null
}

const getPointOfSaleIdFromRegister = (register) => {
  const id = register?.point_of_sale_id ?? register?.pointOfSaleId ?? register?.point_of_sale?.id
  return id != null ? String(id) : null
}

// Fonction clé : récupération de l'ID de caisse à partir d'une vente
const getSaleCashRegisterId = (sale) => {
  // 1. Si la vente a déjà un cash_register_id direct
  const directId = sale?.cash_register_id ?? sale?.cash_register?.id ?? sale?.cashRegisterId
  if (directId != null) return String(directId)

  // 2. Sinon, utiliser le session_id pour retrouver la caisse via la map
  const sessionIdFromSale = sale?.cash_register_session_id ?? sale?.session_id ?? sale?.session?.id
  if (sessionIdFromSale != null) {
    const cashRegId = sessionToCashRegisterMap.value.get(String(sessionIdFromSale))
    if (cashRegId) return cashRegId
  }
  return null
}

const getSalePointOfSaleId = (sale) => {
  const id = sale?.point_of_sale_id ?? sale?.pointOfSaleId ?? sale?.cash_register?.point_of_sale_id ?? sale?.cash_register?.point_of_sale?.id
  return id != null ? String(id) : null
}
...
const formatPointOfSaleName = (pos) => {
  if (!pos) return 'Point de vente'
  return pos?.name || pos?.label || `Point de vente #${getPointOfSaleId(pos) ?? '?'}`
}

// Caisses disponibles selon le point de vente sélectionné
const availableCashRegisters = computed(() => {
  ...
  return cashRegisters.value.filter((register) => {
    ...
    return registerPosId === pointOfSaleFilter.value
  })
})

// Filtrage des ventes (avec conversion stricte en chaîne)
const filteredSales = computed(() => {
  ...
  // Filtre par caisse
  if (cashRegisterFilter.value && cashRegisterFilter.value !== '') {
    filtered = filtered.filter((sale) => {
      const saleRegId = getSaleCashRegisterId(sale)
      return saleRegId != null && String(saleRegId) === String(cashRegisterFilter.value)
    })
  }

  // Filtre par point de vente
  if (pointOfSaleFilter.value && pointOfSaleFilter.value !== '') {
    filtered = filtered.filter((sale) => {
      const salePosId = getSalePointOfSaleId(sale)
      return salePosId != null && String(salePosId) === String(pointOfSaleFilter.value)
    })
  }
...
// Réinitialisation du filtre caisse si le point de vente change
watch(pointOfSaleFilter, () => {
  if (!cashRegisterFilter.value) return
  const isValid = availableCashRegisters.value.some((register) => {
    return getRegisterId(register) === cashRegisterFilter.value
  })
  if (!isValid) {
...
const fetchSales = async (params = {}) => {
  try {
    ...
    const payload = extractArray(data)
...
const fetchCashRegisters = async () => {
  try {
    ...
    const registers = extractArray(data)
    const unique = []
    const seen = new Set()
...
// Chargement du mapping session -> cash_register
const fetchSessionsMapping = async () => {
  try {
    const { data } = await axios.get(`${API_BASE_URL}/cash-register-sessions`, {
      params: { per_page: 2000 }, // assez large pour couvrir toutes les sessions
      headers: authHeaders()
    })
    const sessions = extractArray(data)
    const map = new Map()
    sessions.forEach(session => {
      const sessionId = session.id != null ? String(session.id) : null
      const cashRegId = session.cash_register_id != null ? String(session.cash_register_id) : null
      if (sessionId && cashRegId) {
        map.set(sessionId, cashRegId)
      }
    })
    sessionToCashRegisterMap.value = map
  } catch (error) {
    console.error('Erreur chargement mapping sessions -> caisses:', error)
    // Ne pas bloquer l'application, on pourra afficher un avertissement
  }
}
...
const fetchActiveSession = async () => {
  try {
    ...
    const session = extractArray(data)[0] ?? data?.data ?? data
    if (!session) {
...
onMounted(async () => {
  ...
  try {
    if (isAdmin.value) {
      ...
      // On charge les ventes, les caisses, les points de vente et le mapping session->caisse
      await Promise.allSettled([
        fetchSales(),
        fetchCashRegisters(),
        fetchPointOfSales(),
        fetchSessionsMapping()
      ])
    } else {
      ...
      if (session) {
        ...
        // Pour éviter un appel inutile si l'admin n'est pas connecté, mais on peut le faire aussi
        await fetchSessionsMapping()
      }
    }
  } catch (error) {
    if (!loadError.value) {
